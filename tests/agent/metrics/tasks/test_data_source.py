...
class TestBenchmarkDataSource:
    ...
    def data_source(self, mock_access_token):
        ...
        return BenchmarkDataSource(
            access_token=mock_access_token, repo_id="test/repo", cache_dir="/tmp/test_cache"
        )
    ...
    def data_source_force_download(self, mock_access_token):
        ...
        return BenchmarkDataSource(
            access_token=mock_access_token, repo_id="test/repo", force_download=True
        )
    ...
    def test_validate_access_success(self, mock_hf_api, data_source):
        ...
        mock_api_instance.repo_info.assert_called_once_with(
            repo_id=data_source.repo_id, repo_type=data_source.repo_type
        )
    ...
    def test_get_media_type_unsupported(self, data_source):
        ...
        with patch(
            "fieldworkarena.agent.metrics.tasks.data_source.mimetypes.guess_type"
        ) as mock_guess:
    ...
    def test_load_single_file_txt(
        self, mock_get_media_type, mock_load_base64, mock_download, data_source
    ):
...
class TestBenchmarkDataSourceIntegration:
    ...
    def real_access_token(self):
        ...
        if not token:
            pytest.skip(
                "HF_TOKEN environment variable not set. See tests/README.md for setup instructions."
            )
        return token
    ...
    def test_load_file_payload_multiple_files_real(self, real_data_source):
        ...
        try:
            ...
            result = real_data_source.load_file_payload("6_ScrewTightening_PhoneAssembly.txt")
