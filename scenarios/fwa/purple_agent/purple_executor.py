...
class PurpleExecutor(AgentExecutor):
    ...
    async def _process_request(
        ...
        session = await self._upsert_session(session_id=session_id)  # noqa: F841
    ...
    async def execute(
        ...
        await self._process_request(
            types.UserContent(parts=convert_a2a_parts_to_agent_input(context.message.parts)),
            task.context_id,
            updater,
        )
    ...
    if isinstance(unwrapped_part, FilePart):
        ...
        if isinstance(unwrapped_part.file, FileWithBytes):
            ...
            if (
                ...
                or (file_name and file_name.endswith(".mp4"))
            ):
                try:
                    logger.info(f"Processing video file: {file_name}")
                    frames_parts = process_video_to_parts(file_data, str(file_name))
                    logger.info(
                        f"Extracted {len(frames_parts)} parts from video (including text descriptions)"  # noqa: E501
                    )
                    return frames_parts
            ...
            if (
                ...
                or (file_name and file_name.endswith(".pdf"))
            ):
                try:
            ...
            if (
                ...
                or (file_name and file_name.endswith(".txt"))
            ):
                try:
...
def process_video_to_parts(
    video_data: bytes | str, file_name: str, seconds_per_frame: int = 1
) -> list[types.Part]:
    ...
    try:
        ...
        intro_text = (
            ...
            "You cannot see the video directly, so you MUST use these frames to analyze the video content."  # noqa: E501
        )
...
def extract_video_frames(
    video_path: str, seconds_per_frame: int = 1, max_frames: int = 30
